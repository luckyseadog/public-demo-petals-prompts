...
CONSTANT_TEXT =  """Congratulations on your remarkable achievement in creating true Artificial Intelligence! 
...
class NLPModule:
    def __init__(self, model_name="petals-team/StableBeluga2", plug=False, device="cpu"):
        self.plug = plug
        self.device = device
        ...
        self.model = AutoDistributedModelForCausalLM.from_pretrained(model_name).to(self.device)


    async def inference_websocket(self, data_content, message_deq):
        if not self.plug:
